...
def _argnames(func):
    if callable(func):
        return ' '.join([
            n.upper() for n
            in func.func_code.co_varnames[:func.func_code.co_argcount]])
    else:
        return ''
...
def help():
    ...
    for command in COMMANDS:
        ...
        if callable(func):
            print('    %-40s    %s' % (command + ' ' + _argnames(func),
                                       func.__doc__))
        else:
            print("    %-40s    Returns the value for %s (currently '%s')" % (
                command, command.upper(), resolve(command)))


def story_from_branch(branch_name):
    '''Extracts the story number from a branch name, based on
    the conventions defined in the .dhrc file.'''
...
def format_commit_message(story_num, message):
    '''Formats a commit message, given a story number, based on
    the conventions defined in the .dhrc file.'''
...
if __name__ == "__main__":
    ...
    if prop not in COMMANDS:
        print('Fatal: Unknown command: '+prop, file=sys.stderr)
        help()
        sys.exit(1)
    ...
    if len(args) != argcount:
        print('Usage: %s %s %s' % (sys.argv[0], prop, _argnames(func)),
              file=sys.stderr)
        sys.exit(1)
